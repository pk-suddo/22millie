...
import { Calendar, X, ChevronLeft, ChevronRight, Pencil, Trash2, Plus } from 'lucide-react';
...
export function CalendarPopover() {
  const { income, expenses, deleteIncome, deleteExpense } = useStore();
  ...
  // Add new for a specific date
  const [addingDate, setAddingDate] = useState<string | undefined>();
  ...
  for (let d = 1; d <= daysInMonth; d++) dayFlow[d] = { income: 0, expense: 0 };
  ...
  const openAddExpense = () => {
    ...
    setAddingDate(selDate || undefined);
    setExpenseFormOpen(true);
  };
  const openAddIncome = () => {
  ...
  return (
    <>
              ...
              >
                ...
                <AnimatePresence mode="wait">
                  {view === 'month' ? (
                    ...
                    <motion.div key="day-view" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="p-4 max-h-[70vh] overflow-y-auto">
                      {/* Day header */}
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex gap-1">
                          <button onClick={openAddIncome}
                            className="flex items-center gap-0.5 px-2 py-1 rounded-lg text-[10px] font-bold bg-[#E8FBF6] text-[#00C896] hover:bg-[#C0F5E8] transition-colors">
                            <Plus size={9}/> Income
                          </button>
                          <button onClick={openAddExpense}
                            className="flex items-center gap-0.5 px-2 py-1 rounded-lg text-[10px] font-bold bg-[#FFF0EE] text-[#FF6152] hover:bg-[#FFE0DC] transition-colors">
                            <Plus size={9}/> Expense
                          </button>
                        </div>
                      </div>
                      ...
                      {selFlow && (selFlow.income > 0 || selFlow.expense > 0) ? (
                        <>
                          ...
                          {selIncome.length > 0 && (
                            <div className="mb-3">
                              ...
                              {selIncome.map(i => (
                                <div key={i.id} className="group flex items-center gap-2 py-2 border-b border-[#F5F5F8] last:border-0">
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-[#1A1A2E] truncate">{i.name}</p>
                                    {i.source && <p className="text-xs text-[#9096B4]">{i.source}</p>}
                                  </div>
                                  <span className="font-bold text-[#00C896] text-sm shrink-0">+{formatCompact(i.amount, currency)}</span>
                                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                    <button onClick={() => openEditIncome(i)}
                                      className="p-1.5 rounded-lg bg-[#F0EEFF] text-[#7B61FF] hover:bg-[#7B61FF] hover:text-white transition-colors">
                                      <Pencil size={11}/>
                                    </button>
                                    <button onClick={async () => { if (confirm('Delete this income?')) await deleteIncome(i.id!); }}
                                      className="p-1.5 rounded-lg bg-[#FFF0EE] text-[#FF6152] hover:bg-[#FF6152] hover:text-white transition-colors">
                                      <Trash2 size={11}/>
                                    </button>
                                  </div>
                                </div>
                          ...
                          {selExpenses.length > 0 && (
                            <div>
                              ...
                              {selExpenses.map(e => (
                                <div key={e.id} className="group flex items-center gap-2 py-2 border-b border-[#F5F5F8] last:border-0">
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-[#1A1A2E] truncate">{e.note || e.category}</p>
                                    <p className="text-xs text-[#9096B4]">{e.category}{e.tags?.includes('recurring') ? ' · 🔄' : ''}</p>
                                  </div>
                                  <span className="font-bold text-[#FF6152] text-sm shrink-0">-{formatCompact(e.amount, currency)}</span>
                                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                    <button onClick={() => openEditExpense(e)}
                                      className="p-1.5 rounded-lg bg-[#F0EEFF] text-[#7B61FF] hover:bg-[#7B61FF] hover:text-white transition-colors">
                                      <Pencil size={11}/>
                                    </button>
                                    <button onClick={async () => { if (confirm('Delete this expense?')) await deleteExpense(e.id!); }}
                                      className="p-1.5 rounded-lg bg-[#FFF0EE] text-[#FF6152] hover:bg-[#FF6152] hover:text-white transition-colors">
                                      <Trash2 size={11}/>
                                    </button>
                                  </div>
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-xs text-[#D0CFDD] mt-1">Use the buttons above to add</p>
                        </div>
                      )}
                    </motion.div>
                  )}
      ...
      {/* Edit / Add modals — rendered outside the popover so they aren't clipped */}
      <IncomeForm
        open={incomeFormOpen}
        onClose={() => { setIncomeFormOpen(false); setEditingIncome(undefined); setAddingDate(undefined); }}
        editing={editingIncome}
        defaultDate={addingDate}
      />
      <ExpenseForm
        open={expenseFormOpen}
        onClose={() => { setExpenseFormOpen(false); setEditingExpense(undefined); setAddingDate(undefined); }}
